...
public class Setup_GroupSensorsPage extends BaseClass {
	...
	private void initializeEelements() {
		...
		EditGrpHdrTextBlock = driver.findElementByAccessibilityId("CurrentTextBlock");
	}
...
//Check the enable of  Group Wiring  btn
	...
	//
	public boolean is_SecondGridVisible()
	{
		WebElement ele = driver.findElementByAccessibilityId("SensorGrid2");
		return IsElementVisibleStatus(ele);
	}
	
	public boolean is_sensorVisible(String id)
	{
		WebElement sensor = driver.findElementByAccessibilityId(id);
		return IsElementVisibleStatus(sensor);
		
	}
	public void onVisibleSensor(String id)
	{
		WebElement sensor = driver.findElementByAccessibilityId(id);
		clickOn(sensor);
	}
	public String get_LabelNameOnSensor(String id)
	{
		WebElement sensor = driver.findElementByAccessibilityId(id);
		return FetchText(sensor);
		
	}
	//drog and drop
	public void moving_Sensors(String id1, String id2)
	{
		WebElement sensor = driver.findElementByAccessibilityId(id1);
		WebElement to =  driver.findElementByAccessibilityId(id2);
		
		Actions act = new Actions(driver);
		act.dragAndDrop(sensor, to).build().perform();
	}
	...
	//all text is visible
	public boolean is_AllTextVisibleonPage()
	{
		List<WebElement> txt = driver.findElementsByClassName("TextBlock");
		for(WebElement ele: txt)
		{
			IsElementVisibleStatus(ele);	
		}
		return true;
	}
	//context click
	...
	//	clickOn(EditGroup_Btn);
		WebElement newGrp = driver.findElementByAccessibilityId("ContentElement");
		clickOn(newGrp);
	//	WebElement icon = driver.findElementByAccessibilityId("GlyphElement");
	//	clickOn(icon);
		WebElement Entertxt = driver.findElementByAccessibilityId("txtEditTemp");
	//	WebElement Entertxt = driver.findElementByAccessibilityId("WatermarkTextBlock");
	//	clickOn(Entertxt);
		ClearText(Entertxt);
	...
	//verifying the group list exist or not
	public boolean is_SensorGroupListExixt()
	{
		try
		{
			WebElement grplist = driver.findElementByName("GE.NGV.DataObjects.DataContracts.SetupSensorGroup");
			
			return IsElementVisibleStatus(grplist);
		}
		catch(Exception e)
		{
			return false;
	...
	//click on edit group
	public void click_ReqEditGroup(int num) {
		List<WebElement> EditGroup_Btn = driver.findElementByAccessibilityId("EditGroupButton")
				.findElements(By.className("Button"));
		EditGroup_Btn.get(num);
	}
	// sensors list are displaying that were assigned under Temperature in group
	// Sensor screen Button GroupsListBox

	public String Temp_Sensor_Txt(int num) {
		List<WebElement> senrList = driver.findElementByClassName("GridViewItem")
				.findElements(By.className("TextBlock"));
		return FetchText(senrList.get(num));
	}
	...
	// group name editable
		public boolean is_GroupNameEditable(int Groupnumber) {
			List<WebElement> GroupList = driver.findElementByAccessibilityId("scroll")
					.findElements(By.className("ScrollViewer"));
			// System.out.println("Group nme is"+FetchText(GroupList.get(Groupnumber)));
			GroupList.get(Groupnumber).getAttribute("AutomationId");
			if(GroupList!=null&& GroupList.equals(true))
			{
				return true;
			}
			else
			...
			//return IsElementEnabledStatus(GroupList.get(Groupnumber));GlyphElement
			
		}
		public boolean is_GroupNameEditIconVisible()
		{
		//	WebElement icon = driver.findElementByAccessibilityId("GlyphElement");
			try
			{
			...
			//return IsElementVisibleStatus(icon);
		}
...
//Search and click via sensor name
	...
	public void click_SaveBtn() {
		WebElement SaveBtn_Field = driver.findElementByAccessibilityId("SaveSelectedGroupButton1");
	...
	//get group into new group
	public void click_NewGrpEditor(int num)
	{
		List <WebElement> newGrp = driver.findElementsByAccessibilityId("WatermarkTextBlock");
		clickOn(newGrp.get(num));
		
	}
	//enter text into group
	public void enter_TextinGrpEdtr(String value)
	{
		WebElement newGrp = driver.findElementByAccessibilityId("WatermarkTextBlock");
		enterText(newGrp, value);
	}
	//create new group
	public void create_GroupWithSave(String value)
	{
		clickOn(NewGroupButton);
		...
		WebElement saveBtn = driver.findElementByAccessibilityId("SaveSelectedGroupButton1");
		clickOn(saveBtn);
		
	}
	public void clickon_CreateGroup(String value)
	{
		clickOn(NewGroupButton);
	...
	//click on save
/*	public void click_SaveAfterEdit()
	{
		WebElement saveBtn = driver.findElementByAccessibilityId("SaveSelectedGroupButton1");
		clickOn(saveBtn);
	} */
	//click on new group
	public void enter_textnewGrp(String value)
	{
		WebElement newGrp = driver.findElementByAccessibilityId("ContentElement");
		clickOn(newGrp);
		enterText(newGrp, value);
	...
	//click on new group
		public void  click_OnGroupTitle()
		{
			WebElement newGrp = driver.findElementByAccessibilityId("ContentElement");
			clickOn(newGrp);
			//enterText(newGrp, value);
		}
	//get text on new group
	public String get_TxtNewGroup()
	{
		WebElement newGrp = driver.findElementByAccessibilityId("txtEditTemp");
		newGrp.getAttribute("AutomationId");
		return FetchText(newGrp);
	}
	// Is save btn present
...
//select single / multiple sensors
	...
	public void clickon_MultiSensors() {
		List<WebElement> snsrFields = driver.findElementByAccessibilityId("SensorGrid1")
				.findElements(By.className("GridViewItem"));
			snsrFields.get(0).click();
			snsrFields.get(1).click();
			snsrFields.get(2).click();
			snsrFields.get(3).click();
			snsrFields.get(4).click();
	}
	public void clickon_OtherGroupSensors() {
		List<WebElement> snsrFields = driver.findElementByAccessibilityId("SensorGrid1")
				.findElements(By.className("GridViewItem"));
			snsrFields.get(3).click();
			snsrFields.get(4).click();
	...
	public int count_SelectedSensors(int maxsen) {
		List<WebElement> snsrFields = driver.findElementByAccessibilityId("SensorGrid1")
				.findElements(By.className("GridViewItem"));
		int cnt = 0;
		for(int i=0; i==maxsen; i++)
		{
			if(checkboxSelectStatus(snsrFields.get(i)))
					{
				cnt++;
					}
		}
	...
	public void clickon_ReqSensors(int num) {
		List<WebElement> snsrFields = driver.findElementByAccessibilityId("SensorGrid1")
				.findElements(By.className("GridViewItem"));
			snsrFields.get(num).click();
	}
	public boolean is_SensorSelected(int i)
	{
		List<WebElement> snsrFields = driver.findElementByAccessibilityId("SensorGrid1")
				.findElements(By.className("GridViewItem"));
		return checkboxSelectStatus(snsrFields.get(i));
	}
	...
	public void click_SelectiveListItem(int num)
	{
		List<WebElement> item = driver.findElementByAccessibilityId("GroupsListBox")
				.findElements(By.className("ListBox"));
		item.get(num);
	}
	...
	// click on image 1
	public void click_1stImage()
	{
		WebElement image = driver.findElementByAccessibilityId("ImageButton_0");
		clickOn(image);
	}
	public String get_1stImagePath()
	{
		WebElement image = driver.findElementByAccessibilityId("ImageButton_0");
		// image.getAttribute("AutomationId");
		 return FetchText(image);
		//clickOn(image);
	}
	public void add_1stImage(String path)
	{
		WebElement image = driver.findElementByAccessibilityId("ImageButton_0");
		//clickOn(image);
		enterText(image, path);
	}
	//To verify image is selected or not
	public boolean is_1stImageselected()
	{
		WebElement image = driver.findElementByAccessibilityId("ImageButton_0");
		return checkboxSelectStatus(image);
	}
	...
	public String AlertMsg() {
		WebElement Msg = driver.findElementByAccessibilityId("dispalyMessageTextBlock");
	...
	public int countList_SelectSensors() {
