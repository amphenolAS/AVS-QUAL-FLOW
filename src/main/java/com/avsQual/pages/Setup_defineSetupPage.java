...
public class Setup_defineSetupPage extends BaseClass {
	...
	//setup name field is enabled
	public boolean SetupNameEnabledState() {
		return IsElementEnabledStatus(DefineSetup_SetupName_txtBx);
	}
	// click Setup Name
	...
	public String GetAttributesetupName(String ele) {
//		WebElement Sensor_Val = driver.findElementByAccessibilityId("PART_TextBox");
		 String Attribute_Val = DefineSetup_SetupName_txtBx.getAttribute(ele);
		return FetchText(DefineSetup_SetupName_txtBx);
	}
	public String GetAttributeOfSOP(String ele) {
//		WebElement Sensor_Val = driver.findElementByAccessibilityId("PART_TextBox");
		 String Attribute_Val = DefineSetup_SOP_txtBx.getAttribute(ele);
	...
	//Check the Sensor COunt Field is Enabled or not
	public boolean setupSensorCountField_Enabled()
	{
		return IsElementEnabledStatus(DefineSetup_Sensordata_txtBx);
	}
	...
	{
		DefineSetup_Sensordata_txtBx.sendKeys(Keys.ENTER);
	}
	public String GetAttributeSensrText(String ele) {
		// DefineSetup_Sensordata_txtBx.getAttribute(ele);
		WebElement Sensor_Val = driver.findElementByAccessibilityId("PART_TextBox");
		 String Attribute_Val = Sensor_Val.getAttribute(ele);
	...
	public String GetAttributeOfLDes(String ele) {
		 String Attribute_Val = DefineSetup_LoadDesc_txtBx.getAttribute(ele);
	...
	// Enter Sensor count data
	public Setup_SensorConfigPage enter_MandatoryFieldsindefineSetupconfigPage(String setUpNm, String sensorCnt )
	...
	public String GetAttributeOfComments(String ele) {
		 String Attribute_Val = DefineSetup_Cmnts_txtBx.getAttribute(ele);
	...
	public void Click_Help_Icon_AppBar() throws InterruptedException { 
		Actions ac = new Actions(driver);
		ac.contextClick(DefineSetup_AssetID_txtBx).build().perform();

		WebElement bottomMenu_AppHelp_Icon = driver.findElementByAccessibilityId("HelpAppBarButton");
		clickOn(bottomMenu_AppHelp_Icon);
		Thread.sleep(1000);
	}
	//verify Helpscreen is visible or not
	public boolean is_HelpBarDisplayed()
	{
	WebElement help_Bar = driver.findElementByAccessibilityId("helpFlyout");
	return IsElementVisibleStatus(help_Bar);
	...
	//checking Define setup page is whether truncated/overlapped
	public boolean validatePageTruncation()
	{
//		Long pageHeight = (Long) ((JavascriptExecutor) driver).executeScript("return window.innerHeight;");
//	    Long pageWidth = (Long) ((JavascriptExecutor) driver).executeScript("return window.innerWidth;");
		Dimension currentDimension = driver.manage().window().getSize();
		int height = currentDimension.getHeight();
		int width = currentDimension.getWidth();
		WebElement ele = driver.findElementByName("Advanced Validation System");
		
		Dimension elementsize = ele.getSize();
		Point elementposition = ele.getLocation();
		//System.out.println(elementsize); //(1366, 728) 
		//System.out.println(elementposition); //(-8, -8)
		
		int expectedHeight = (1366*728);
		int expectedY = (-8*-8);
		
		if 
		(elementsize.getHeight() < expectedHeight || elementposition.getY() + elementsize.getHeight() > height
	            || elementposition.getX() + elementsize.getWidth() > width) {
	            return false;
	        }
